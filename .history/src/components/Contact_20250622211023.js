...
function Contact() {
  ...
  const contactData = {
    name: faker.name.findName(),
    phone: `+91 ${faker.datatype.number({ min: 1000, max: 9999 })} ${faker.datatype.number({ min: 100, max: 999 })} ${faker.datatype.number({ min: 100, max: 999 })}`,
    avatar: faker.image.avatar(),
    about: faker.lorem.sentence(),
    groupAvatar: faker.image.avatar(),
    groupName: faker.company.companyName() + " Group",
      ...
      >
        <Stack spacing={3} p={3}>
          ...
          <Stack spacing={2}>
            <Button
              variant="text"
              endIcon={<CaretRight size={16} color={isDark ? "#aaa" : "#666"} />}
              sx={{
