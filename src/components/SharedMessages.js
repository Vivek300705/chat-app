...
function SharedMessages({ onBackToContact }) {
  ...
  const handleBackClick = () => {
    ...
    if (onBackToContact && typeof onBackToContact === "function") {
      console.log("Calling onBackToContact"); // Debug log
      onBackToContact();
    } else {
      console.warn("onBackToContact is not provided or not a function");
      // Fallback: you might want to dispatch an action or navigate differently
      // For example, you could dispatch a Redux action to go back
      // dispatch(someBackAction());
    }
  };
